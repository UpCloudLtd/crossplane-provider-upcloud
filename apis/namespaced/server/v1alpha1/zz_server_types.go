...
type AdditionalIPAddressInitParameters struct {
	...
	// (String) The primary IP address of this interface.
...
type NetworkInterfaceInitParameters struct {
	...
	// 31 blocks of additional IP addresses to assign to this interface. Allowed only with network interfaces of type private (see below for nested schema)
	...
	// assigned when the server is updated. This might result to inaccurate diffs in the plan, when interfaces are re-ordered or when interface is removed from the middle of the list.
	// The interface index.
...
type ServerInitParameters struct {
	...
	// (Number) The number of CPU cores for the server
	...
	HotResize *bool `json:"hotResize,omitempty" tf:"hot_resize,omitempty"`
	...
	// (Block List) One or more blocks describing the network interfaces of the server.
	...
	// (Block Set) Simple backup schedule configuration
	...
	// (String) A short, informational description of the server.
...
type StorageDevicesInitParameters struct {
	...
	// +crossplane:generate:reference:type=github.com/UpCloudLtd/crossplane-provider-upcloud/apis/namespaced/storage/v1alpha1.Storage
	Storage *string `json:"storage,omitempty" tf:"storage,omitempty"`

	// Reference to a Storage in storage to populate storage.
	// +kubebuilder:validation:Optional
	StorageRef *v1.NamespacedReference `json:"storageRef,omitempty" tf:"-"`

	// Selector for a Storage in storage to populate storage.
	// +kubebuilder:validation:Optional
	StorageSelector *v1.NamespacedSelector `json:"storageSelector,omitempty" tf:"-"`
...
// Server is the Schema for the Servers API. The UpCloud server resource allows the creation, update and deletion of cloud servers https://upcloud.com/products/cloud-servers. -> To deploy a GPU server, select a plan with GPU- prefix, e.g., GPU-8xCPU-64GB-1xL40S. Use upctl zone devices command to list per zone GPU availability.
