...
type MySQLIncrementalBackupInitParameters struct {

	// efficient than full backups. This is particularly useful for large databases where daily full backups would be too time-consuming or expensive.
	...
	// separated list of days of the week when full backups should be created. Valid values: mon, tue, wed, thu, fri, sat, sun.
	...
	FullBackupWeekSchedule *string `json:"fullBackupWeekSchedule,omitempty" tf:"full_backup_week_schedule,omitempty"`
}

type MySQLIncrementalBackupObservation struct {
...
type MySQLIncrementalBackupParameters struct {
...
type PropertiesInitParameters struct {
	...
	// (String) The slow log output destination when slow_query_log is ON. To enable MySQL AI Insights, choose INSIGHTS. To use MySQL AI Insights and the mysql.slow_log table at the same time, choose INSIGHTS,TABLE. To only use the mysql.slow_log table, choose TABLE. To silence slow logs, choose NONE.
	...
	LogOutput *string `json:"logOutput,omitempty" tf:"log_output,omitempty"`

	// (Number) The slow_query_logs work as SQL statements that take more than long_query_time seconds to execute.
	...
	// (Boolean) Slow query log enables capturing of slow queries. Setting slow_query_log to false also truncates the mysql.slow_log table.
